...
classes = ['buffalo', 'elephant', 'rhino', 'zebra', 'fox', 'tiger', 'wolf', 'lion', 'hyena', 'cheetah']  

# 3. Build model architecture and load weights
model = models.resnet18(weights=None)
model.fc = torch.nn.Linear(model.fc.in_features, len(classes))
model.load_state_dict(torch.load("resnet_classifier_wildlife.pt", map_location=device))
model.to(device)
model.eval()
